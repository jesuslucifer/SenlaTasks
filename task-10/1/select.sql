...
SELECT maker, speed FROM laptop JOIN product ON laptop.model = product.model WHERE ram > 99999;

#7.Найти номера моделей и цены всех продуктов (любого типа), выпущенных производителем B (латинская буква).
SELECT product.model, price FROM laptop JOIN product ON laptop.model = product.model WHERE maker = 'B'
UNION SELECT product.model, price FROM printer JOIN product ON printer.model = product.model WHERE maker = 'B'
...
SELECT AVG(speed) FROM laptop WHERE price > 999;
